...
import { VISIBLE_NOTICE_STATE_ID, type AdminNotice } from "@/modules/notices/domain/types";

interface NoticeActionsMenuProps {
  notice: AdminNotice;
  onEdit: (notice: AdminNotice) => void;
  onToggleVisibility: (notice: AdminNotice) => void;
...
export function NoticeActionsMenu({
  notice,
  onEdit,
  onToggleVisibility,
  onDelete,
}: NoticeActionsMenuProps) {
  ...
  function run(action: (notice: AdminNotice) => void) {
    setOpen(false);
    action(notice);
  }
